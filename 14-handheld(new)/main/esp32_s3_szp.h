/**
 * @file esp32_s3_szp.h
 * @brief 立创实战派 ESP32-S3 开发板 (SZP) 板级支持包 (BSP)
 *
 * 定义开发板所有外设的 GPIO 引脚映射、I2C 设备地址、寄存器定义和初始化函数。
 *
 * 硬件外设：
 * - I2C 总线 (SDA=GPIO1, SCL=GPIO2) — 共享给 IMU/Codec/触摸/摄像头
 * - QMI8658 六轴 IMU (I2C addr: 0x6A)
 * - PCA9557 IO 扩展器 (I2C addr: 0x19) — 控制 LCD_CS/PA_EN/DVP_PWDN
 * - ST7789 LCD 320x240 (SPI3, MOSI=GPIO40, CLK=GPIO41, DC=GPIO39, BL=GPIO42)
 * - FT5x06 电容触摸 (I2C)
 * - GC0308 摄像头 (DVP 接口, XCLK=GPIO5)
 * - ES8311 音频 DAC (I2C addr: 0x18, I2S 输出到喇叭)
 * - ES7210 音频 ADC (I2C addr: 0x41, I2S 输入从麦克风)
 * - SD 卡 (SDMMC 1-bit, CMD=GPIO48, CLK=GPIO47, DAT0=GPIO21)
 * - SPIFFS 文件系统 (存储 MP3 资源)
 *
 * 适配 ESP-IDF v5.5.1
 */
#pragma once
